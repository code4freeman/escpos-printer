...
class Command {
    ...
    textRow (texts = []) {
        const
            width = Math.floor(this._textLength / (this._state.fontSize + 1)),
            cellWidth = Math.floor(width / texts.length);
        let
            text = "",
            moreRow = 0;
        texts.forEach((c, index) => {
            let cWidth = this._computeTextLength(c),
                rowNum = Math.ceil(cWidth / cellWidth);
            rowNum = Math.ceil((rowNum * 2 /* 乘以2，加入的间隔按中文长度来计算 */ + cWidth) / cellWidth) - 1;
            if (rowNum > moreRow) moreRow = rowNum;
            const { i, str } = readCountStr.call(this, c, cellWidth - 1); //减去1，保留一个字符的间距
            text += str;
